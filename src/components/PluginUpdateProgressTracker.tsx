...
export const PluginUpdateProgressTracker: React.FC<{
    ...
    if (githubRateLimitTimestamp) {
        ...
        errorInstructions = `Your IP address has exceeded github's limit of 60 file downloads in an hour. The limit will reset in ${time.fromNow()}, and if that doesn't fix it then report an issue`;
    } else {
                ...
                <DivCompletedNextSteps>
                    ...
                    {failureCount === 0 && (
                        <div>
                            ...
                            )} successfully installed and reloaded!`}</p>
                        </div>
                    )}
                    {failureCount > 0 && (
                        <div>
